...
class DubboClient(object):
    ...
    def __init__(self, interface, version='', dubbo_version='2.6.3', zk_register=None, host=None, group=None):
        ...
        if group is not None:
            ...
            self.__group = ''
        self.__path = self.__group + self.__interface
    ...
    def call(self, method, args=(), timeout=None):
        ...
        if self.__zk_register:  # 优先从zk中获取provider的host
            host = self.__zk_register.get_provider_host(self.__path)
        else:
            host = self.__host
...
class ZkRegister(object):
    ...
    def __init__(self, hosts, auth_data=None, application_name='search_platform'):
        """
        :param hosts: Zookeeper的地址
        :param application_name: 当前客户端的名称
        """
        zk = KazooClient(hosts=hosts, auth_data=auth_data)
        # 对zookeeper连接状态的监控
    ...
    def _routing_with_wight(self, interface):
        ...
        :return:
        """
        hosts = list(self.hosts[interface])
        if not hosts:
            raise RegisterException('no providers for interface {}'.format(interface))
        # 此接口没有权重设置，使用朴素的路由算法
